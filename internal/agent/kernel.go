...
type ChatRequest struct {
	...
	// Attachments 是本轮消息携带的图片或文件，数据使用原始字节保存。
...
func (k *Kernel) Run(ctx context.Context, request ChatRequest) iter.Seq2[*session.Event, error] {
	...
	return func(yield func(*session.Event, error) bool) {
		...
		runner, compactionTracker, err := k.buildRunner(ctx, resolved, tools, runtime, memoryService, projectInstructions, request.InvocationID, taskContract, runtimeSnapshot, workingSet, toolSetSnapshot, negotiation.RequestPlan)
		if err != nil {
			yield(nil, err)
			return
		}
...
func (k *Kernel) warmRuntimeToolCatalog(ctx context.Context, runtime RuntimeOptions, workspaceID, conversationID string) error {
