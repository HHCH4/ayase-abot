...
func TestConfigCenterAPIProfilesBindingsAndSystemSettings(t *testing.T) {
	...
	settings := callHTTP(handler, http.MethodPut, "/api/v1/system-settings", `{"log_level":"debug","request_timeout_seconds":60}`)
	if settings.Code != http.StatusOK || !strings.Contains(settings.Body.String(), `"log_level":"debug"`) {
